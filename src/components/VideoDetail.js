...
const VideoDetail = () => {
  ...
  // const {snippet : { title , channelId , channleTitle }, statistics : { viewCount, likeCount }} = videoDetail
  return (
    <Box minHeight="95vh">
          ...
          <Box sx={{ width: '100%', position: 'sticky', top: '86px' }}>
            ...
            <Stack direction='row' justifyContent='space-between' sx={{ color: '#fff' }} py={1} px={2}>
              <Link to={`/channel/${channelid}`}>
                <Typography>
                  {videoDetail && videoDetail.snippet ? (<Typography variant={{ sm: 'subtitle1', md: 'h6' }} color='#fff'>
